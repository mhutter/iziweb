...
class ImportForm extends React.Component {
  initialState () { return { value: '', data: [], error: null } }
  ...
  handleSubmit (event) {
    ...
    finance.createTransactions(this.state.data)
      .then(
        transactions => {
  ...
  render () {
    return (
      <div className='ImportForm modal fade' tabIndex='-1' id='import-data-modal'>
        <div className='modal-dialog modal-xl'>
          <form onSubmit={e => this.handleSubmit(e)} className='modal-content'>
            ...
            <div className='modal-body'>
              ...
              <TransactionList>
                {this.state.data.map((t, i) => <Transaction key={i} {...t} />)}
              </TransactionList>
            </div>
            <div className='modal-footer'>
              <button
                ...
                disabled={this.state.error || this.state.data.length < 1}
              >
                Daten importieren
              </button>
            </div>
